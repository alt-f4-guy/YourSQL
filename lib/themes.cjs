...
function seedThemes(directory,sources) {
  if (fs.existsSync(directory)) return;
  fs.mkdirSync(directory,{recursive:true});
  for (const source of sources) {
    let entries;
    try {entries=fs.readdirSync(source,{withFileTypes:true});} catch {continue;}
    for (const entry of entries) {
      const destination=path.join(directory,entry.name);
      if (entry.isFile() && entry.name.toLowerCase().endsWith('.json') && !fs.existsSync(destination)) fs.copyFileSync(path.join(source,entry.name),destination);
    }
  }
}
...
module.exports = {readThemes,deleteTheme,seedThemes};
