...
const {assertAvailable}=require('./storage.cjs');
...
class PracticeService {
  ...
  bootstrap() {
    assertAvailable(this.store.storage);
    return {problems:this.entries.map(({pack,problem:p})=>({id:p.id,level:p.level,title:p.title,topic:p.topic,description:p.description,columns:p.columns,ordered:p.ordered,starter:p.starter,hints:p.hints,
      ...
      progress:this.store.progress,logs:this.store.logs(),engine:{ready:this.engine.ready,message:this.engine.message,missing:this.engine.missing},dataPath:this.directory};
  }
  ...
  saveDraft({id,sql,review=false,reviewDue=null}) { this.entry(id); return this.store.saveDraft(id,sql,review,reviewDue); }
  async run({id,sql}) {
  ...
  async submit({id,sql,review=false,reviewDue=null}) {
    const {pack,problem} = this.entry(id);
    return this.exclusive(async()=>{
      this.store.saveDraft(id,sql,review,reviewDue);
      const start = Date.now();
      ...
      try {
        ...
        // 서버 준비·기준 쿼리 오류는 학습자의 오답으로 처리하지 않는다.
        result.answerError=answering&&error.answerError===true;
