...
class Updater {
  ...
  async install() {
    ...
    try {
      ...
      const encoded=Buffer.from(fs.readFileSync(helper,'utf8'),'utf16le').toString('base64');
      const launcher=path.join(work,'launcher.cmd');
      if (process.platform==='win32') fs.writeFileSync(launcher,'@echo off\r\n"%YOURSQL_UPDATE_POWERSHELL%" -NoProfile -NonInteractive -EncodedCommand %YOURSQL_UPDATE_COMMAND% > "%YOURSQL_UPDATE_OUTPUT%" 2>&1\r\nexit /b %errorlevel%\r\n');
      const args=process.platform==='darwin'?[helper,target,candidate,transaction.backup,transaction.failed,transaction.ready,this.resultFile,String(process.pid)]:['/d','/s','/c',launcher];
      // 시작 전 PowerShell 오류도 작업 폴더 정리와 무관하게 보존한다.
      const helperOutput=path.join(this.app.getPath('userData'),'update-helper-output.txt');
      let output;
      if (process.platform==='win32') fs.writeFileSync(helperOutput,'',{mode:0o600});
      else output=fs.openSync(helperOutput,'w',0o600);
      let child;
      // Windows PowerShell은 직접 분리하면 실행되지 않으므로 cmd를 분리하고 그 자식으로 실행한다.
      try {child=spawn(command,args,{detached:true,stdio:process.platform==='win32'?'ignore':['ignore',output,output],cwd:work,windowsHide:true,env:{...process.env,YOURSQL_UPDATE_TRANSACTION:path.join(work,'transaction.json'),YOURSQL_UPDATE_POWERSHELL:powershell,YOURSQL_UPDATE_COMMAND:encoded,YOURSQL_UPDATE_OUTPUT:helperOutput}});}
