...
class Updater {
  ...
  async install() {
    ...
    try {
      ...
      let child;
      // Windows PowerShell은 콘솔을 분리하면 명령 실행 없이 종료될 수 있다.
      try {child=spawn(command,args,{detached:process.platform!=='win32',stdio:['ignore',output,output],cwd:work,windowsHide:true,env:{...process.env,YOURSQL_UPDATE_TRANSACTION:path.join(work,'transaction.json')}});}
