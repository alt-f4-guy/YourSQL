...
class Updater {
  ...
  async install() {
    ...
    try {
      ...
      const command=process.platform==='darwin'?'/bin/sh':powershell;
      const args=process.platform==='darwin'
        ?[helper,target,candidate,transaction.backup,transaction.failed,transaction.ready,this.resultFile,String(process.pid)]
        :['-NoProfile','-NonInteractive','-EncodedCommand',Buffer.from(fs.readFileSync(helper,'utf8'),'utf16le').toString('base64')];
      ...
      try {
        child=spawn(command,args,{
          ...
          cwd:work,
          windowsHide:true,
          env:{...process.env,YOURSQL_UPDATE_TRANSACTION:path.join(work,'transaction.json')}
        });
