...
class Updater {
  ...
  async install() {
    ...
    try {
      ...
      if (process.platform==='win32') fs.writeFileSync(launcher,'@echo off\r\n"%YOURSQL_UPDATE_POWERSHELL%" -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "%YOURSQL_UPDATE_HELPER%" > "%YOURSQL_UPDATE_OUTPUT%" 2>&1\r\nexit /b %errorlevel%\r\n');
      ...
      let output;
      if (process.platform==='darwin') output=fs.openSync(helperOutput,'w',0o600);
      else fs.writeFileSync(helperOutput,'',{mode:0o600});
      let child;
      // Windows PowerShell은 직접 분리하면 실행되지 않으므로 cmd를 분리하고 그 자식으로 실행한다.
      try {child=spawn(command,args,{detached:true,stdio:process.platform==='win32'?'ignore':['ignore',output,output],cwd:work,windowsHide:true,env:{...process.env,YOURSQL_UPDATE_TRANSACTION:path.join(work,'transaction.json'),YOURSQL_UPDATE_POWERSHELL:powershell,YOURSQL_UPDATE_HELPER:helper,YOURSQL_UPDATE_OUTPUT:helperOutput}});}
