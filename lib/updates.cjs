...
class Updater {
  ...
  async install() {
    ...
    try {
      ...
      const command=process.platform==='darwin'?'/bin/sh':path.join(process.env.SystemRoot||'C:\\Windows','System32','WindowsPowerShell','v1.0','powershell.exe');
      const args=process.platform==='darwin'?[helper,target,candidate,transaction.backup,transaction.failed,transaction.ready,this.resultFile,String(process.pid)]:['-NoProfile','-NonInteractive','-EncodedCommand',Buffer.from(fs.readFileSync(helper,'utf8'),'utf16le').toString('base64')];
      // 보조 프로그램의 작업 디렉토리도 교체 대상 밖으로 둔다.
      const child=spawn(command,args,{detached:true,stdio:'ignore',cwd:work,windowsHide:true,env:{...process.env,YOURSQL_UPDATE_TRANSACTION:path.join(work,'transaction.json')}});
      helperChild=child;
      ...
      for (let attempt=0;!fs.existsSync(path.join(work,'armed'));attempt++) {
        if (attempt>=100 || child.exitCode!==null) throw new Error('업데이트 보조 프로그램을 시작하지 못했습니다.');
      ...
      if (!handedOff) {
        this.busy=false;
        if (helperChild && helperChild.exitCode===null) helperChild.kill();
