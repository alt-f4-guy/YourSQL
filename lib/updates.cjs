...
class Updater {
  ...
  async install() {
    ...
    try {
      ...
      if (process.platform==='win32') fs.writeFileSync(launcher,'@echo off\r\n"%YOURSQL_UPDATE_POWERSHELL%" -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "%YOURSQL_UPDATE_HELPER%"\r\nexit /b %errorlevel%\r\n');
      ...
      try {child=spawn(command,args,{detached:true,stdio:['ignore',output,output],cwd:work,windowsHide:true,env:{...process.env,YOURSQL_UPDATE_TRANSACTION:path.join(work,'transaction.json'),YOURSQL_UPDATE_POWERSHELL:powershell,YOURSQL_UPDATE_HELPER:helper}});}
