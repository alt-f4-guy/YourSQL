...
function createReminderRuntime({app,Notification,powerMonitor,shell,onOpen,onChanged=()=>{},recover=true}){
  ...
  const reminders=new Reminders({directory:app.getPath('userData'),scheduler,notify,recover});
  ...
  return {reminders,check,
    ...
    async monitor(){const current=generation;await reminders.reconcile();if(current!==generation)return;await check();if(current!==generation)return;if(!timer){timer=setInterval(resume,30000);powerMonitor.on('resume',resume);}},
    dispose(){generation++;clearInterval(timer);timer=undefined;powerMonitor.removeListener('resume',resume);}
  };
}
