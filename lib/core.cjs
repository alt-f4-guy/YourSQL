...
const {readStore,writeStore,assertAvailable,validateProgress,validateLog}=require('./storage.cjs');
...
class PracticeStore {
  constructor(directory) {
    ...
    this.storage=readStore(this.file,{validate:validateProgress,defaultValue:{}});
    this.progress=this.storage.value;this.logIssues=[];
  }
  saveDraft(id, sql, review=false, reviewDue=null) {
    ...
    if (reviewDue!==null && (typeof reviewDue!=='string' || !/^\d{4}-\d{2}-\d{2}$/.test(reviewDue))) throw new Error('복습 초안의 날짜가 잘못되었습니다.');
    // 예정일이 같은 회차의 초안만 복원하며 일반 학습 초안과 분리한다.
    this.progress[id] = {...this.progress[id],...(review?{reviewSql:sql,reviewDue}:{sql})};
    writeStore(this.file,this.progress,{validate:validateProgress});
    return {ok:true};
  }
  ...
  logs() {
    this.logIssues=[];
    const logs=[];
    for(const name of fs.readdirSync(this.logDirectory).filter(f=>f.endsWith('.json'))){
      const result=readStore(path.join(this.logDirectory,name),{validate:validateLog,recover:false});
      if(result.status==='ok')logs.push(result.value);else this.logIssues.push(...result.issues);
    }
