...
class Engine {
  ...
  async start() {
    ...
    this.starting = this.initialization.catch(async error=>{
  ...
  async initialize() {
    ...
    if (!fs.existsSync(data)) {
      ...
      await new Promise((resolve,reject)=>{
        let child;
        child=execFile(binary,['--no-defaults','--initialize-insecure',`--datadir=${staging}`,`--log-error=${log}`],{timeout:90000,windowsHide:true},error=>{
          if(this.initializing===child) this.initializing=null;
          error?reject(error):resolve();
        });
        this.initializing=child;
      });
  ...
  async query(sql) {
    ...
    if (typeof sql !== 'string' || sql.length > 50000 || !sql.trim()) throw Object.assign(new Error('SQL을 1~50,000자 사이로 작성하세요.'),{answerError:true});
    const leading = sql.replace(/^(?:\s|--[^\n]*(?:\n|$)|#[^\n]*(?:\n|$)|\/\*[\s\S]*?\*\/)+/,'');
    if (!/^(SELECT|WITH)\b/i.test(leading)) throw Object.assign(new Error('조회용 SELECT 또는 WITH 쿼리만 실행할 수 있습니다.'),{answerError:true});
    const connection = mysql.createConnection({socketPath:this.socketPath,user:'learner',password:this.password,database:'practice',
      // Windows 파이프는 Unix 소켓과 달리 비밀번호의 RSA 교환이 필요하다.
      authPlugins:windows?{caching_sha2_password:mysql.authPlugins.caching_sha2_password({overrideIsSecure:false})}:undefined,
    ...
    try {
      ...
      return await new Promise((resolve,reject)=>{
        ...
        const abort = async message => {
          ...
          connection.destroy();
          reject(Object.assign(new Error(message),{answerError:true}));
        };
        ...
        // SQLSTATE가 있는 문장 오류만 오답이며 연결·프로토콜 오류는 제외한다.
        query.on('error',error=>finish(Object.assign(new Error(error.errno === 3024 || error.errno === 1317 ? '실행 제한 시간 3초를 초과하여 중단했습니다.' : `MySQL 오류: ${error.message}`),{answerError:Boolean(error.sqlState)&&!error.sqlState.startsWith('08')&&!error.fatal})));
        query.on('end',()=>finish());
      });
  ...
  stop() {
    ...
    if (initializing && initializing.exitCode === null && initializing.signalCode === null) {
      await new Promise(resolve=>{
        const timer=setTimeout(()=>initializing.kill('SIGKILL'),5000);
        initializing.once('exit',()=>{clearTimeout(timer);resolve();});
        initializing.kill('SIGTERM');
      });
    }
    // 이 실행이 소유한 초기화 자식의 종료를 확인한 뒤에만 미완성 데이터를 지운다.
    if (initializing) fs.rmSync(path.join(this.directory,'mysql-initializing'),{recursive:true,force:true});
    if (this.initializing===initializing) this.initializing=null;
    // 연결 대기가 끝나기 전에 자식을 죽이면 Windows 감시 프로세스 뒤에 서버가 남는다.
    // 시작 오류의 정리와 서로 기다리지 않도록 원래 초기화 작업만 기다린다.
    if (this.initialization) await this.initialization.catch(()=>{});
    this.ready=false;
    let shutdown=false;
    if (this.admin) {
      try { await this.admin.query({sql:'SHUTDOWN',timeout:2000}); shutdown=true; } catch {}
      this.admin.destroy();
      this.admin=null;
    }
    ...
    if (child && child.exitCode === null && child.signalCode === null) {
      await new Promise(resolve=>{
        ...
        // Windows에서는 감시 프로세스를 먼저 죽이면 실제 서버가 종료 중인 채 남는다.
        if (!windows || !shutdown) child.kill('SIGTERM');
      });
    }
    this.child=null;
