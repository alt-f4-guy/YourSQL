...
const {readStore,writeStore,assertAvailable,validateLearning}=require('./storage.cjs');
...
// 구형 records가 비어 있어도 실제 배정의 정답 이력은 완료 증거다. 원본은 변경하지 않는다.
function passedIds(data){
  const passed=new Set(Object.entries(data.records||{}).filter(([,r])=>r?.passed===true).map(([id])=>id));
  for(const d of Object.values(data.days||{}))for(const c of [...(d.completedCycles||[]),d]){
    for(const id of c.done||[])if(c.blanks?.includes(id))passed.add(id);
    if(Array.isArray(c.queries)){for(const id of c.queriesDone||[])if(c.queries.includes(id))passed.add(id);}
    else if(c.queryDone===true&&c.query)passed.add(c.query);
  }
  return passed;
}
const completeWith=(passed,unit)=>unit.cards.every(c=>passed.has(c.id))&&unit.queries.every(id=>passed.has(id));
function unitComplete(data,unit){return completeWith(passedIds(data),unit);}
function courseComplete(data){const passed=passedIds(data);return units.every(unit=>completeWith(passed,unit));}
function learningGoal(data,date){
  ...
  return {goalCycles,completedCycleCount:count,goalComplete:count>=goalCycles,
    courseComplete:courseComplete(data)};
}
...
class Learning {
  constructor(directory,clock=()=>new Date(),wrongAnswers=[]) {
    ...
    const source=(fs.existsSync(this.file)||fs.existsSync(this.file+'.bak'))?this.file:path.join(directory,'learning-v2.json');
    this.storage=readStore(source,{validate:validateLearning,defaultValue:{days:{},records:{}}});
    this.data=this.storage.value;
    if(this.storage.status==='blocked')return;
  ...
  save(){assertAvailable(this.storage);writeStore(this.file,this.data,{validate:validateLearning});}
  ...
  assignment(date){
    const passed=passedIds(this.data);
    const unit=units.find(u=>!completeWith(passed,u));
  ...
  today(){
    ...
    let changed=false;
    if(today.courseComplete&&!courseComplete(this.data)){
      const replacement=this.assignment(date);
      // 완료 플래그 때문에 비워진 오늘 배정만 복원한다. 기존 완료 사이클은 유지한다.
      if(!today.blanks.length)Object.assign(today,replacement,{goalCycles:storedDailyCycles(today.goalCycles)||this.dailyCycles()});
      today.courseComplete=false;changed=true;
    }
  ...
  startExtra(){
    ...
    if(cycleComplete(current)&&!courseComplete(this.data)){
  ...
  snapshot(){
    ...
    const hasMore=!courseComplete(this.data),decoratedToday={...today,...learningGoal(this.data,date),currentCycleComplete:cycleComplete(today)};
...
module.exports={Learning,learningGoal,cycleComplete,unitComplete,courseComplete};
