...
class Learning {
  ...
  today(){
    ...
    if(!this.data.days[date]){
      ...
      this.data.days[date]={date,unit:unit.id,blanks:chosen.map(c=>c.id),query,done:[],queryDone:false};
      this.save();
    }
  ...
  snapshot(){
    ...
    const history=Object.fromEntries(Object.entries(this.data.days).map(([date,d])=>{
      const blankCount=new Set(d.done.filter(id=>d.blanks.includes(id))).size,queryDone=Boolean(d.queryDone);
      return [date,{blankCount,queryDone,complete:blankCount===3&&queryDone}];
    }));
    return {today,history,records:this.data.records,units:units.map(({items,cards,...unit})=>({...unit,cards:cards.map(({answer,explanation,...card})=>card)})),
