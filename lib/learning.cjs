...
class Learning {
  ...
  snapshot(){
    ...
    const history=Object.fromEntries(Object.entries(this.data.days).map(([date,d])=>{
      ...
      const goalCycles=storedDailyCycles(d.goalCycles),completedCycleCount=cycles.filter(cycleComplete).length;
      const goalComplete=goalCycles===null?completedCycleCount>0:completedCycleCount>=goalCycles;
      return [date,{blankCount,queryCount,queryDone:queryCount>0,total:blankCount+queryCount,complete:goalComplete,goalComplete,goalCycles,completedCycles:goalCycles===null?null:completedCycleCount,completedCycleCount:goalCycles===null?null:completedCycleCount,cycleDetailAvailable:goalCycles!==null}];
    }));
    const hasMore=cards.some(c=>!this.data.records[c.id]?.passed),todayHistory=history[date],decoratedToday={...today,completedCycleCount:todayHistory.completedCycleCount??0,currentCycleComplete:cycleComplete(today),goalComplete:todayHistory.complete,courseComplete:Boolean(today.courseComplete)||!hasMore&&cycleComplete(today)};
