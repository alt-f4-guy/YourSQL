...
class Reminders{
  ...
  reload(recover=true){
    ...
    this.storage=readStore(this.file,{validate:validateReminders,defaultValue:defaults,recover,recoveryValue:value=>{
      const date=localDate(this.clock());
      return {...defaults,...value,enabled:false,lastAttemptDate:date,attemptedDates:[...new Set([...(value.attemptedDates||[]),date])],
        lastError:'알림 설정을 복원하여 알림을 껐습니다. 설정에서 다시 켜 주세요. 복구 당일 자동 알림은 보류합니다.'};
    }});
