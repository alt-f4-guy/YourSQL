...
function themeDirectories(executable,userData,platform=process.platform) {
  const paths=platform==='win32'?path.win32:path.posix;
  const legacy=paths.resolve(paths.dirname(executable),platform==='darwin'?'../../..':'.','theme');
  return {active:platform==='darwin'?paths.join(userData,'theme'):legacy,legacy};
}
