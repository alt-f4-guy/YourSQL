...
function themeDirectories(executable,userData,platform=process.platform,paths=path) {
  const legacy=platform==='darwin'
    ? paths.join(paths.dirname(paths.resolve(paths.dirname(executable),'../..')),'theme')
    : paths.join(paths.dirname(executable),'theme');
  return {active:paths.join(userData,'theme'),legacy};
}
...
module.exports={themeDirectories,mysqlCandidates,validSocket};
