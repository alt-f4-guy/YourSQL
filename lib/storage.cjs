...
function validateLearning(v){
 ...
 for(const [key,d] of Object.entries(v.days)){
  requireShape(date(key)&&object(d)&&date(d.date)&&d.date===key&&(d.completedCycles===undefined||Array.isArray(d.completedCycles)));
...
function readStore(file,{validate,defaultValue={},recover=true,recoveryValue=value=>value}={}){
 let primaryError;
 ...
 try{
  ...
  const restored=checked(recoveryValue(previous.value),validate);
  writeSynced(temporary,JSON.stringify(restored,null,2));fs.renameSync(temporary,file);syncDirectory(path.dirname(file));
  issue.recoveredAt=new Date().toISOString();
  return {status:'recovered',value:restored,issues:[issue]};
