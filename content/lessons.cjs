...
const units = [
  ...
  {title:'범위와 정렬',query:'level1_02',concept:'BETWEEN은 양 끝을 포함한 범위입니다. ORDER BY로 정렬하며 DESC는 내림차순입니다.',items:[
    ['범위 조건','가격이 1000 이상 3000 이하인 상품의 이름을 조회하세요.','SELECT name FROM products WHERE price ___ 1000 AND 3000','BETWEEN','BETWEEN은 경곗값 1000과 3000도 포함합니다.'],
    ['정렬 절','가격이 낮은 상품부터 정렬하세요.','SELECT name, price FROM products ___ price','ORDER BY','ORDER BY로 정렬 기준을 지정하며, 방향을 생략하면 오름차순입니다.'],
  ...
  {title:'중복과 개수',query:'level1_05',concept:'DISTINCT는 출력 조합의 중복을 제거합니다. COUNT(*)는 행 수를 세고 COUNT(열)는 NULL을 제외합니다.',items:[
    ...
    ['고유값 개수','NULL을 제외한 서로 다른 도시의 수를 구하세요.','SELECT COUNT(___ city) FROM customers','DISTINCT','COUNT(DISTINCT city)는 NULL을 제외한 서로 다른 도시 수입니다.']]},
  {title:'문자열 패턴',query:'level1_07',concept:'LIKE에서 %는 길이 0 이상의 문자열, _는 한 글자를 뜻합니다. 문자열 패턴은 따옴표로 감쌉니다.',items:[
    ['패턴 비교','이름에 ‘노’가 들어간 상품을 고르세요.',"SELECT name FROM products WHERE name ___ '%노%'",'LIKE','LIKE는 와일드카드를 포함한 문자열 패턴을 비교합니다.'],
    ['접두어','‘김’으로 시작하는 이름을 고르세요. 따옴표 안의 패턴을 입력하세요.',"SELECT name FROM customers WHERE name LIKE '___'",'김%','뒤의 %는 김 뒤에 어떤 문자열이 와도 된다는 의미입니다.'],
    ['한 글자','‘김’으로 시작하는 세 글자 이름을 고르세요. 빈칸에 필요한 밑줄을 입력하세요.',"SELECT name FROM customers WHERE name LIKE '김___'",'__','밑줄 하나는 한 글자이므로 두 개가 필요합니다.']]},
  {title:'그룹별 집계',query:'level2_01',concept:'GROUP BY는 같은 값을 가진 행을 묶습니다. COUNT·AVG 등의 집계 함수로 그룹별 값을 계산합니다.',items:[
    ...
    ['반올림','평균 가격을 소수 셋째 자리에서 반올림해 둘째 자리까지 표시하세요.','SELECT ___(AVG(price), 2) FROM products','ROUND','ROUND(값, 2)는 소수 셋째 자리에서 반올림해 둘째 자리까지 남깁니다.']]},
  ...
  {title:'NULL과 집계 결합',query:'level2_03',concept:'COALESCE로 NULL을 ‘미입력’으로 바꿔 표시합니다. 이 표현식으로 그룹화하면 표시되는 도시 이름별로 고객 수를 셀 수 있습니다.',items:[
    ['그룹 표시값','도시가 NULL인 경우 ‘미입력’으로 표시하세요.',"SELECT ___(city, '미입력') AS city_label FROM customers",'COALESCE','NULL일 때 사용할 표시값을 두 번째 인수로 줍니다.'],
    ['열 별칭','출력 열 이름을 city_label로 지정하세요.',"SELECT COALESCE(city, '미입력') ___ city_label FROM customers",'AS','AS는 출력 열에 이름을 붙입니다.'],
    ['동일 표현식 그룹화','화면에 표시할 도시 이름별로 고객 수를 구하세요.',"SELECT COALESCE(city, '미입력'), COUNT(*) FROM customers ___ COALESCE(city, '미입력')",'GROUP BY','출력에 사용한 도시 표현식으로 묶습니다.']]},
  {title:'조건별 값 만들기',query:'level2_05',concept:'CASE는 위에서부터 조건을 검사하여 처음 맞는 THEN 값을 반환합니다. ELSE는 어떤 조건에도 맞지 않을 때 사용합니다.',items:[
    ['조건 분기 시작','가격에 따라 ‘저가’ 또는 ‘기타’를 반환하는 식을 시작하세요.',"SELECT ___ WHEN price < 1000 THEN '저가' ELSE '기타' END FROM products",'CASE','CASE는 조건에 따라 값을 반환하는 표현식입니다.'],
    ...
    ['기본값','저가가 아닌 상품에 기타를 표시하세요.',"SELECT CASE WHEN price < 1000 THEN '저가' ___ '기타' END FROM products",'ELSE','ELSE를 생략하면 참인 WHEN 조건이 없을 때 NULL을 반환합니다.']]},
  {title:'날짜 조건 조합',query:'level1_04',concept:'날짜·시간 범위는 시작 이상, 다음 구간 시작 미만으로 쓰면 마지막 날의 시간까지 포함할 수 있습니다.',items:[
    ['동시 조건','결제 완료 상태이며 주문일이 2024년 1월 1일이거나 그 이후인 주문을 고르세요.',"SELECT order_id FROM orders WHERE status = 'paid' ___ ordered_at >= '2024-01-01'",'AND','두 조건을 모두 만족해야 하므로 AND로 연결합니다.'],
    ['끝 경계','주문일이 2024년 1월에 속하도록 끝 날짜의 조건을 채우세요.',"SELECT order_id FROM orders WHERE ordered_at >= '2024-01-01' AND ordered_at ___ '2024-02-01'",'<','다음 달 시작 미만으로 제한하면 시간값이 있어도 1월 전체를 포함합니다.'],
