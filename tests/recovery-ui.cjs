...
(async()=>{
 for(const name of ['learning.json','progress.json']){
  ...
  let app;try{
   ...
   app=await electron.launch({args:[root],env:{...process.env,SQL_PRACTICE_DATA_DIR:directory,YOURSQL_TEST_HIDDEN:'1'},timeout:60000});
   ...
   await page.locator('#retry-storage').click();await page.waitForFunction(()=>!document.getElementById('start-daily').disabled);
