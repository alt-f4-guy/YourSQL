    ...
    await app.evaluate(({app},fixture)=>{
      ...
      if(fixture.helperFailure){
        ...
        fs.copyFileSync=(source,target,...args)=>{
          ...
          if(/helper\.(sh|ps1)$/.test(target))fs.writeFileSync(target,process.platform==='win32'?"Write-Output 'helper-startup-test'; [System.Console]::Error.WriteLine('helper-startup-test'); exit 42":"#!/bin/sh\nprintf 'helper-startup-test' >&2\nexit 42\n");
        };
      }
