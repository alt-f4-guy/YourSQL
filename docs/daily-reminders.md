...
- Windows Server 2025 x64 CI에서 동일 소스의 핵심·실제 MySQL·설치·재설치·제거·알림 화면·기록 복구·배포 ZIP 검사를 통과했습니다. 실제 사용자 데스크톱의 알림 배너·OS 클릭·절전 후 동작은 미검증입니다.
...
종료가 SQL 실행이나 초안 저장 실패로 보류되어도 감시는 계속 유지됩니다. Windows 제거는 정상 종료를 먼저 요청하고 응답하지 않으면 재시도·강제 종료·취소를 제공합니다. 무인 제거의 강제 종료는 `/S /FORCE`를 명시한 경우에만 허용합니다. Windows CI에서 설치·제거 및 알림 설정·종료 보류 후 감시·초안 보존 검사가 통과했습니다. 실제 배너·OS 클릭 등 남은 항목은 검증 기록에서 구분합니다.
...
NSIS의 기본 `Uninstall.exe` 실행기는 임시 제거 프로세스를 띄운 뒤 종료하므로 실행기 코드만으로 제거 성공을 판단하지 않습니다. 자동화에서 최종 종료 코드가 필요하면 `Uninstall.exe`를 별도 임시 폴더에 복사한 뒤, 그 복사본에 `/S _?=<원래 설치 경로>`를 전달하고 기다립니다. `_?=`는 따옴표 없이 마지막 인수로 전달합니다. 강제 종료를 허용할 때만 `/S /FORCE _?=<원래 설치 경로>`를 사용합니다. 실제 제거 프로세스가 0을 반환한 뒤 설치 폴더·예약·등록이 제거되었는지도 확인합니다. 실패하면 원래 설치 위치의 제거 프로그램으로 다시 시도할 수 있습니다.

[NSIS 공식 제거 실행 설명](https://nsis.sourceforge.io/When_I_use_ExecWait_uninstaller.exe_it_doesn't_wait_for_the_uninstaller)을 따릅니다. CI도 같은 방식으로 실패·재시도 결과를 검사합니다.
