(() => {
  ...
  async function execute(kind) {
    ...
    try { const data = await api[kind]({ id, sql, review: state.reviewId === id, reviewDue: state.reviewDue }); if (kind === 'submit' && data.progress) { state.progress[id] = data.progress; renderList(); } if (state.current?.id !== id) return; if (kind === 'run') renderResult(data); else { renderGrade(data); if (data.logId && data.status !== 'correct') el.grade.prepend(node('div', 'auto-log', '오답 기록에 자동 저장됨')); if(data.status==='wrong'||data.answerError===true||(data.status==='correct'&&state.reviewId===id)){const notice=await window.learningUI?.reviewNotice(id);if(notice)el.grade.prepend(node('p','auto-log',notice));} renderProblem(state.current); } }
