...
function calendarMonth(year,month,today,history){
  ...
  const days=Array.from({length:new Date(year,month+1,0).getDate()},(_,i)=>{
    ...
    const blankCount=record?.blankCount||0,queryDone=Boolean(record?.queryDone),total=blankCount+Number(queryDone);
    return {date,blankCount,queryDone,total,recorded:Boolean(record),status:date>today?'future':total===4?'complete':total?'partial':'none'};
  });
